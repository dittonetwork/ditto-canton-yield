...
| **Tokenized RWA tranche** | Phase 8 marketplace tier — SPV-wrapped tokenized fund instruments (FOBXX money-market, DLR repo, HQLAX collateral, HSBC TDS, plus private-credit-fund LP tranches). Available as standalone KYC-gated dvTokens (`dvFOBXX`, `dvCredit-A`, …), not as a leg inside the permissionless Core/Locked vaults. See README — Tokenized Fund Marketplace and ARCHITECTURE §13. |
...
The strategy framework above supports a lineup of vault products, each its own CIP-56 dvToken with its own fee schedule and strategy mix. The lineup is split across two access tiers:

### Permissionless retail tier
...
| **`dvUSDCx-LOCK90`** *(Phase 7)* | USDCx | 90 days | Legs 1–4 with looser buffer, higher Leg 2 weight | 6–11% modeled; same gating | 1.5% mgmt / 15% carry |
| **`dvUSDCx-LOCK1Y`** *(Phase 7)* | USDCx | 1 year | Legs 1–4, higher leverage | 7–12% modeled; same gating | 2.0% mgmt / 20% carry |
| **`dvCC`** *(Phase 7)* | CC | Daily-liquid | Alpend CC supply + future CC strategies | TBD | 1.0% mgmt / 10% carry |
| **`dvCBTC`** *(Phase 7)* | CBTC | Daily-liquid | Alpend CBTC supply + future CBTC strategies | TBD | 1.0% mgmt / 10% carry |

### KYC-gated marketplace tier *(Phase 8)*

KYC at user signup unlocks the marketplace shelf. Each product wraps an underlying tokenized fund instrument issued on Canton, via a per-fund SPV. NAV is computed from DVN-attested fund-admin reports (see ARCHITECTURE §13).

| Vault | Underlying | Liquidity | Target APY | Indicative fees |
|---|---|---|---|---|
| **`dvFOBXX`** | Franklin Templeton FOBXX (tokenized money-market) | Daily at fund level; product-defined window | ~5% (current FOBXX yield) | 0.5% mgmt / no carry |
| **`dvDLR`** | Broadridge DLR (repo collateral) | Issuer-defined | 4–6% | 0.5% mgmt / no carry |
| **`dvHQLAX`** | HQLAX (high-quality liquid assets) | Issuer-defined | 3–5% | 0.5% mgmt / no carry |
| **`dvTDS`** | HSBC TDS (time deposit) | Term-locked | 3–5% | 0.5% mgmt / no carry |
| **`dvCredit-A`, `dvCredit-B`, …** | SPV-wrapped private credit fund LP tranches | Quarterly redemption windows | 10–14% target | 1.5% mgmt / 15% carry |

Marketplace yield is **not** generated by the platform's strategy router; it flows through from the underlying fund. The router-driven strategy legs (Legs 1–4) apply only to the permissionless retail tier.
